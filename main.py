...
def root():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute('SELECT productId, name, price, description, image, stock FROM products where productId = 17 or productId = 18 or productId = 26 or productId = 27 or productId = 28')
        likeData = cur.fetchall()
        cur.execute('SELECT categoryId, name FROM categories')
        categoryData = cur.fetchall()
    itemData = parse(itemData)
    likeData = parse(likeData)
    return render_template('home.html', itemData=itemData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData, likeData=likeData)
